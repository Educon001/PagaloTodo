...
public class GetPaymentsByServiceIdQueryHandlerTest
{
    ...
    public async void GetPaymentsByServiceIdQueryHandle_Returns_List()
    {
        ...
        var expectedResponse = service.Payments!.Select(p=>PaymentMapper.MapEntityToResponse(p,true,false)).ToList();
        var query = new GetPaymentsByServiceIdQuery(service.Id,DateTime.MinValue,DateTime.MaxValue);
